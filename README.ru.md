...
```text
qwen36-35b-a3b/
├── README.md / README.ru.md   <- этот файл и его русская версия
...
Соглашение о языках: у каждого человекочитаемого документа есть английская
версия `*.md` и русская `*.ru.md` со строкой-переключателем сверху.
`AGENT_PROMPT.md` намеренно только на английском — он написан для агента.
Подробные отчёты с результатами в `docs/` пока только на английском.

## Что внутри и где читать подробнее

| Тема | Документ |
| --- | --- |
| Железо и версии ПО | `HARDWARE.ru.md` |
| Полная хронология и причины | `CHRONOLOGY.ru.md` |
| Что влияет на prefill/decode/VRAM | `WHAT_INFLUENCES_WHAT.ru.md` |
| Запуск / пересборка / откат / диагностика | `RUNBOOK.ru.md` |
| Задание для агента: собрать и проверить автоматически | `AGENT_PROMPT.md` |
| DP2A A/B на трёх-GPU профиле | `docs/cmp50hx-dp2a-df03399-result.md` |
| DP2A + `-fmad=false` A/B | `docs/cmp50hx-dp2a-no-fmad-df03399-result.md` |
| Split / ubatch / размещение MTP | `docs/cmp50hx-split-ubatch-mtp-df03399-result.md` |
| RTX-хвост + глубина MTP (итог) | `docs/cmp50hx-rtx-tail-mtp-df03399-result.md` |
| Контекстные лесенки 262K/360K | `docs/cmp50hx-context-ladder-262k-360k.md` |
| Дизайн-сессии (агентный режим, 360K) | `docs/cmp50hx-design-session-360k.md`, `docs/cmp50hx-design-sessions-go-2apps.md` |
